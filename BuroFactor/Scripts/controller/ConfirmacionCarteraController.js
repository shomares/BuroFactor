"use strict";
var ConfirmacionCarteraController = function ($scope, $uibModal, $rootScope, $location, $window, TokenFactory, ConfirmacionCarteraFactory) {
    $scope.todosProveedores = true;
    ...
    $scope.End = false;
    ConfirmacionCarteraFactory.addManejadorEventos("onError", function (args, func) {
    ...
    $scope.$on('WindowOpened', function (event, args) {
        if ($scope.lastWindow !== undefined) {
            if (!$scope.lastWindow.closed) $scope.lastWindow.close();
        }
        $scope.lastWindow = args.window;
        $scope.lastWindow.focus();
    });
    ConfirmacionCarteraFactory.addManejadorEventos("onConfirm", function (args) {
        var modalInstance = $uibModal.open({
            backdrop: 'static',
            keyboard: false,
            animation: true,
            templateUrl: window.location.origin + ConfirmacionCarteraFactory.Config.rootURL + "Principal/Token?id=" + args,
        ...
        modalInstance.result.then(function (args) {
            ...
            if (args.notificacion === undefined)
                ...
                message = args.notificacion;
            alertify.alert($scope.logoSuccess + "<p style='font-size:large'>" + message + "</p></div>",
                function () {
    ...
    ConfirmacionCarteraFactory.addManejadorEventos("onLoadFacturas", function (args) {
        $scope.lista = args;
        $scope.seleccionarTodos = false;
        if ($scope.lista !== undefined) {
            if ($scope.lista.length === 0) {
                alertify.alert($scope.logoInformacion + "<p style='font-size:large'>No existen documentos disponibles para cancelar.</p></div>");
            } else {
                $scope.displayTable = {
                    'display': 'block'
                };
            ...
            alertify.alert($scope.logoInformacion + "<p style='font-size:large'>No existen documentos disponibles para confirmar.</p></div>");
        }
        $scope.displayLoading = {
            'display': 'none'
        };
    });
    ConfirmacionCarteraFactory.addManejadorEventos("onLoad", function (args) {
        $scope.proveedores = args.proveedoresAut;
        $scope.proveedor = "";
        $scope.divisa = "";
    ...
    $scope.buscarFacturas = function () {
        ...
        if ($scope.todosProveedores) {
            if ($scope.todasDivisas) {
                $scope.displayLoading = {
                    'display': 'block'
                };
                ConfirmacionCarteraFactory.buscarFacturas($scope.proveedor, $scope.FechaInicial, $scope.FechaFinal, $scope.divisa);
            } else {
                if ($scope.divisa !== "") {
                    $scope.displayLoading = {
                        'display': 'block'
    ...
    $scope.confirmar = function (idValue) {
        $scope.displayLoading = {
            'display': 'block'
        };
        ConfirmacionCarteraFactory.confirmar(idValue);
    };
    $scope.enviar = function () {
        ...
        if (num === 0) {
            alertify.alert($scope.logoInformacion + "<p style='font-size:large'>Seleccione por lo menos un elemento de la lista.</p></div>");
        } else {
            $scope.numeroFac = num;
            $scope.displayCorrecto = {
                'display': 'block'
    ...
    $scope.confirmarTodos = function () {
        ...
        ConfirmacionCarteraFactory.confirmarLista(listaele);
    };
    ConfirmacionCarteraFactory.load();
    $scope.borraSeleccionProveedor = function () {
    ...
    $scope.seleccionarTodo = function () {
        $scope.lista.forEach(function (factura) {
            factura.selected = $scope.seleccionarTodos;
        });
    };
...
ConfirmacionCarteraController.$inject = ['$scope', '$uibModal', '$location', '$rootScope', '$window', 'TokenFactory', 'ConfirmacionCarteraFactory'];
root.factory('ConfirmacionCarteraFactory', ConfirmacionCarteraFactory);
root.factory('TokenFactory', TokenFactory);
root.controller('ConfirmacionCarteraController', ConfirmacionCarteraController);