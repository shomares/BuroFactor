"use strict";
var OperacionVencidaController = function ($scope, $location, $window, $uibModal, OperacionVencidaFactory) {
    ...
    OperacionVencidaFactory.addManejadorEventos("onLoadFacturas", function (args) {

        var jBrowser = new JBrowser();
        $scope.error = "";
        if (args.state === 1) {
            if (jBrowser.mobileAndTabletcheck()) {
                window.location.href = OperacionVencidaFactory.Config.rootURL + "AspNetForms/aspnetViewver.aspx?generaPDF=true";
            } else if (jBrowser.isIE) {
                var origin = window.location.origin + window.location.pathname;
                $window.open(OperacionVencidaFactory.Config.rootURL + "AspNetForms/aspnetViewver.aspx?generaPDF=true", '_blank');
                ...
                var modalInstanceVisor = $uibModal.open({
                    ...
                    keyboard: false,
                    animation: true,
                    templateUrl: OperacionVencidaFactory.Config.rootURL + "Reporte/Visor",
                    size: 'lg',
                    resolve: {
                        url: function () {
                            return OperacionVencidaFactory.Config.rootURL + "AspNetForms/aspnetViewver.aspx";
                        }
                    },
    ...
    OperacionVencidaFactory.addManejadorEventos("onError", function (args, func) {
        $scope.End = true;
        $scope.displayLoading = {
            'display': 'none'
        };
    ...
    OperacionVencidaFactory.addManejadorEventos("onLoad", function (args) {
    ...
    $scope.buscarFacturas = function () {
        ...
        if ($scope.todosempresas) {
            if ($scope.todasDivisas) {
                ...
                OperacionVencidaFactory.buscarFacturas($scope.emisor, $scope.FechaInicial, $scope.FechaFinal, $scope.divisa);
            } else {
                if ($scope.divisa !== "") {
                    $scope.displayLoading = {
                        'display': 'block'
    ...
    $scope.eliminar = function (idValue) {
        ...
        OperacionVencidaFactory.eliminar(idValue);
    };
...
OperacionVencidaController.$inject = ['$scope', '$location', '$window', '$uibModal', 'OperacionVencidaFactory'];
root.factory('OperacionVencidaFactory', OperacionVencidaFactory);
root.controller("VisorController", VisorController);
root.controller('OperacionVencidaController', OperacionVencidaController);
