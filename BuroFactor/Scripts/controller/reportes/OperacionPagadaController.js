"use strict";
var OperacionPagadaController = function ($scope, $location, $window, $uibModal, OperacionPagadaFactory) {
    ...
    OperacionPagadaFactory.addManejadorEventos("onLoadFacturas", function (args) {

        var jBrowser = new JBrowser();
        $scope.error = "";
        if (args.state === 1) {
            if (jBrowser.mobileAndTabletcheck()) {
                window.location.href = OperacionPagadaFactory.Config.rootURL + "AspNetForms/aspnetViewver.aspx?generaPDF=true";
            } else if (jBrowser.isIE) {
                var origin = window.location.origin + window.location.pathname;
                $window.open(OperacionPagadaFactory.Config.rootURL + "AspNetForms/aspnetViewver.aspx?generaPDF=true", '_blank');
                ...
                var modalInstanceVisor = $uibModal.open({
                    ...
                    keyboard: false,
                    animation: true,
                    templateUrl: OperacionPagadaFactory.Config.rootURL + "Reporte/Visor",
                    size: 'lg',
                    resolve: {
                        url: function () {
                            return OperacionPagadaFactory.Config.rootURL + "AspNetForms/aspnetViewver.aspx";
                        }
                    },
    ...
    OperacionPagadaFactory.addManejadorEventos("onError", function (args, func) {
        $scope.End = true;
        $scope.displayLoading = {
            'display': 'none'
        };
    ...
    OperacionPagadaFactory.addManejadorEventos("onLoad", function (args) {
    ...
    $scope.buscarFacturas = function () {
        ...
        if ($scope.todosempresas) {
            if ($scope.todasDivisas) {
                ...
                OperacionPagadaFactory.buscarFacturas($scope.emisor, $scope.FechaInicial, $scope.FechaFinal, $scope.divisa);
            } else {
                if ($scope.divisa !== "") {
                    $scope.displayLoading = {
                        'display': 'block'
    ...
    $scope.eliminar = function (idValue) {
        ...
        OperacionPagadaFactory.eliminar(idValue);
    };
...
OperacionPagadaController.$inject = ['$scope', '$location', '$window', '$uibModal', 'OperacionPagadaFactory'];
root.factory('OperacionPagadaFactory', OperacionPagadaFactory);
root.controller("VisorController", VisorController);
root.controller('OperacionPagadaController', OperacionPagadaController);
