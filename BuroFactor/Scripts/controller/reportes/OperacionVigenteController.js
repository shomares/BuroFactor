"use strict";
var OperacionVigenteController = function ($scope, $location, $window, $uibModal, OperacionVigenteFactory) {
    ...
    OperacionVigenteFactory.addManejadorEventos("onLoadFacturas", function (args) {

        var jBrowser = new JBrowser();
        $scope.error = "";
        if (args.state === 1) {
            if (jBrowser.mobileAndTabletcheck()) {
                window.location.href = OperacionVigenteFactory.Config.rootURL + "AspNetForms/aspnetViewver.aspx?generaPDF=true";
            } else if (jBrowser.isIE) {
                var origin = window.location.origin + window.location.pathname;
                $window.open(OperacionVigenteFactory.Config.rootURL + "AspNetForms/aspnetViewver.aspx?generaPDF=true", '_blank');
                ...
                var modalInstanceVisor = $uibModal.open({
                    ...
                    keyboard: false,
                    animation: true,
                    templateUrl: OperacionVigenteFactory.Config.rootURL + "Reporte/Visor",
                    size: 'lg',
                    resolve: {
                        url: function () {
                            return OperacionVigenteFactory.Config.rootURL + "AspNetForms/aspnetViewver.aspx";
                        }
                    },
    ...
    OperacionVigenteFactory.addManejadorEventos("onError", function (args, func) {
        $scope.End = true;
        $scope.displayLoading = {
            'display': 'none'
        };
    ...
    OperacionVigenteFactory.addManejadorEventos("onLoad", function (args) {
    ...
    $scope.buscarFacturas = function () {
        ...
        if ($scope.todosempresas) {
            if ($scope.todasDivisas) {
                ...
                OperacionVigenteFactory.buscarFacturas($scope.empresa, $scope.FechaInicial, $scope.FechaFinal, $scope.divisa);
            } else {
                if ($scope.divisa !== "") {
                    $scope.displayLoading = {
                        'display': 'block'
    ...
    $scope.eliminar = function (idValue) {
        ...
        OperacionVigenteFactory.eliminar(idValue);
    };
...
OperacionVigenteController.$inject = ['$scope', '$location', '$window', '$uibModal', 'OperacionVigenteFactory'];
root.factory('OperacionVigenteFactory', OperacionVigenteFactory);
root.controller("VisorController", VisorController);
root.controller('OperacionVigenteController', OperacionVigenteController);
