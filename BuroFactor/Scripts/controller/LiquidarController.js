"use strict";
var LiquidarController = function ($scope, $location, $window, $uibModal, TokenFactory, LiquidarFactory) {
    ...
    LiquidarFactory.addManejadorEventos("onDelete", function (args) {
        var childWindow;
        var modalInstance = $uibModal.open({
            backdrop: 'static',
            keyboard: false,
            animation: true,
            templateUrl: window.location.origin + LiquidarFactory.Config.rootURL + "Operacion/Token",
    ...
    LiquidarFactory.addManejadorEventos("onLoadFacturas", function (args) {
        $scope.lista = args;
        $scope.seleccionarTodos = false;
        if ($scope.lista !== undefined) {
            if ($scope.lista.length === 0) {
                alertify.alert($scope.logoInformacion + "<p style='font-size:large'>No existen deudas disponibles para eliminar.</p></div>");
            } else {
                $scope.displayTable = {
                    'display': 'block'
                };
    ...
    LiquidarFactory.addManejadorEventos("onError", function (args, func) {
        $scope.End = true;
        $scope.displayLoading = {
            'display': 'none'
        };
    ...
    LiquidarFactory.addManejadorEventos("onLoad", function (args) {
    ...
    $scope.buscarFacturas = function () {
        ...
        if ($scope.todosempresas) {
            if ($scope.todasDivisas) {
                ...
                LiquidarFactory.buscarFacturas($scope.empresa, $scope.FechaInicial, $scope.FechaFinal, $scope.divisa);
            } else {
                if ($scope.divisa !== "") {
                    $scope.displayLoading = {
                        'display': 'block'
    ...
    $scope.eliminar = function (idValue) {
        ...
        LiquidarFactory.eliminar(idValue);
    };
        ...
        $scope.lista.forEach(function (factura) {
            if (factura.selected) {
                listaele.push({
                    ...
                    idProveedor: factura.idEmisor,
                    monto: factura.monto
                });
            }
        });
        LiquidarFactory.eliminarLista(listaele);
    };
    LiquidarFactory.load();
...
LiquidarController.$inject = ['$scope', '$location', '$window', '$uibModal', 'TokenFactory', 'LiquidarFactory'];
root.factory('LiquidarFactory', LiquidarFactory);
root.factory('TokenFactory', TokenFactory);
root.controller('LiquidarController', LiquidarController);