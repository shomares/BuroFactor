"use strict";
var CancelarController = function ($scope, $location, $window, $uibModal, TokenFactory, CancelarFactory) {
    ...
    CancelarFactory.addManejadorEventos("onDelete", function (args) {
        var childWindow;
        var modalInstance = $uibModal.open({
            backdrop: 'static',
            keyboard: false,
            animation: true,
            templateUrl: window.location.origin + CancelarFactory.Config.rootURL + "Operacion/Token",
    ...
    CancelarFactory.addManejadorEventos("onLoadFacturas", function (args) {
        $scope.lista = args;
    ...
    CancelarFactory.addManejadorEventos("onError", function (args, func) {
        $scope.End = true;
        $scope.displayLoading = {
            'display': 'none'
        };
    ...
    CancelarFactory.addManejadorEventos("onLoad", function (args) {
    ...
    $scope.buscarFacturas = function () {
        ...
        if ($scope.todosempresas) {
            if ($scope.todasDivisas) {
                ...
                CancelarFactory.buscarFacturas($scope.empresa, $scope.FechaInicial, $scope.FechaFinal, $scope.divisa);
            } else {
                if ($scope.divisa !== "") {
                    $scope.displayLoading = {
                        'display': 'block'
    ...
    $scope.eliminar = function (idValue) {
        ...
        CancelarFactory.eliminar(idValue);
    };
    ...
    $scope.eliminarTodos = function () {
        ...
        CancelarFactory.eliminarLista(listaele);
    };
    CancelarFactory.load();
...
CancelarController.$inject = ['$scope', '$location', '$window', '$uibModal', 'TokenFactory', 'CancelarFactory'];
root.factory('CancelarFactory', CancelarFactory);
root.factory('TokenFactory', TokenFactory);
root.controller('CancelarController', CancelarController);